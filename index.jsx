...
const PALETTES = {
  light: {
    bg: "#F6F8FF", surface: "#FFFFFF", surfaceSoft: "#EEF2FF", border: "#D9E0F2",
    text: "#15162B", textSoft: "#525A78", textFaint: "#8790AD",
    ink: "#10152F", inkSoft: "#1A2250", inkBorder: "#27316A", inkText: "#F8FAFF", inkTextSoft: "#AAB4E0",
    accent: "#FF4D6D", accentSoft: "#FFE7ED", accentText: "#D92850", accentBorder: "#FFB7C7",
    teal: "#00B8D9", tealSoft: "#DDF8FF", tealText: "#007D98",
    danger: "#E53935", dangerSoft: "#FFE8E7", dangerText: "#B71C1C",
    success: "#16A05D", successSoft: "#E1F8EC",
  },
  dark: {
    bg: "#090D1F", surface: "#121A33", surfaceSoft: "#182344", border: "#29365F",
    text: "#F7F9FF", textSoft: "#AEB9DA", textFaint: "#7180A8",
    ink: "#060A18", inkSoft: "#101A38", inkBorder: "#24345F", inkText: "#F8FAFF", inkTextSoft: "#9AA9D1",
    accent: "#FF4D6D", accentSoft: "#3A1726", accentText: "#FF8FA5", accentBorder: "#6E2941",
    teal: "#19D3E6", tealSoft: "#102F3A", tealText: "#67E7F2",
    danger: "#FF5B57", dangerSoft: "#3B171A", dangerText: "#FF9B98",
    success: "#34D399", successSoft: "#123528",
  },
};
const BADGE_COLORS = [
...
export default function App() {
  ...
  const [profile, setProfile] = useState({ name: "Moosa Lebelo", email: "moosa.lebelo@company.com", role: "Operations Manager" });
  ...
  useEffect(() => {
    (async () => {
      try {
        const value = window.localStorage.getItem("workai-state");
        if (value) {
  ...
  useEffect(() => {
    if (!loaded) return;
    const t = setTimeout(() => {
      try { window.localStorage.setItem("workai-state", JSON.stringify({ profile, aiPrefs, tasks, history, chatMessages, themeMode })); } catch (e) {}
    }, 500);
      ...
      <div style={{ fontFamily: "Inter, sans-serif", background: pal.bg, color: pal.text, minHeight: 600, display: "flex", position: "relative", borderRadius: 16, overflow: "hidden", border: `1px solid ${pal.border}` }}>
        <style>{`
          ...
          * { box-sizing: border-box; }
          ...
          input:focus, textarea:focus, select:focus { border-color: ${pal.accent} !important; box-shadow: 0 0 0 3px ${pal.accentSoft}; }
        ...
        <main style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
          <div style={{ display: "none", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderBottom: `1px solid ${pal.border}` }} className="waiTopbarMobile">
            ...
            <span style={{ fontFamily: "Fraunces, serif", fontWeight: 600, fontSize: 13 }}>WORK PRODUCTIVITY AI</span>
            <div style={{ width: 20 }} />
          </div>
  ...
  return (
    <>
      <div style={{ display: "flex", alignItems: "center", gap: 9, padding: "6px 10px 20px" }}>
        ...
        <div>
          <div style={{ fontFamily: "Fraunces, serif", fontWeight: 600, fontSize: 14.5, color: pal.inkText, lineHeight: 1.15, letterSpacing: 0.3 }}>WORK PRODUCTIVITY AI</div>
          <div style={{ fontSize: 10, color: pal.inkTextSoft, letterSpacing: 0.3 }}>by Moosa Lebelo</div>
        </div>
      </div>
...
function WorkplaceChat({ chatMessages, setChatMessages, addHistory, toast, prefill, setPrefill }) {
  ...
  const system = `ROLE: You are Workplace AI, a helpful, realistic workplace productivity assistant embedded in the Work Productivity AI application.
...
function ResponsibleAI() {
  ...
  return (
    <div className="waiFadeIn">
      <PageHeader title="Responsible AI" subtitle="How Work Productivity AI approaches AI safety, accuracy, and human oversight." />
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }} className="waiTwoCol">
        {sections.map(s => (
